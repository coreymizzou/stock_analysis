...
def fetch_social_sentiment(ticker):
    try:
        ...
        query = f"https://nitter.net/search?f=tweets&q={ticker}%20stock&since=&until=&near=&within=15mi"
        response = requests.get(query, headers=headers, timeout=10)
        if response.status_code != 200:
            print(f"Failed to fetch X data for {ticker}: HTTP {response.status_code}")
            return 0
        ...
        print(f"Error fetching X sentiment for {ticker}: {e}")
        return 0
